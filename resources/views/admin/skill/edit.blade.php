...
@section('content')
<h1 class="text-2xl font-bold gradient-text mb-8">Edit Skill</h1>
@if($errors->any())
    <div class="mb-4 bg-red-500/20 border border-red-500 text-red-400 rounded-lg px-4 py-2">
        {{ $errors->first() }}
    </div>
@endif
<form method="POST" action="{{ route('admin.skill.update', $skill->_id) }}" class="space-y-6">
    @csrf
    @method('PUT')
    <div>
        <label class="block text-light font-semibold mb-2">Nama Skill</label>
        <input type="text" name="name" value="{{ old('name', $skill->name) }}" class="w-full px-4 py-3 bg-dark border border-primary/30 rounded-lg text-light placeholder-gray-400 focus:outline-none focus:border-primary transition-colors duration-300" required>
    </div>
    <div>
        <label class="block text-light font-semibold mb-2">Level (1-100)</label>
        <input type="number" name="level" value="{{ old('level', $skill->level) }}" min="1" max="100" class="w-full px-4 py-3 bg-dark border border-primary/30 rounded-lg text-light placeholder-gray-400 focus:outline-none focus:border-primary transition-colors duration-300" required>
    </div>
    <div>
        ...
        <select name="category" class="w-full px-4 py-3 bg-dark border border-primary/30 rounded-lg text-light focus:outline-none focus:border-primary transition-colors duration-300" required>
            ...
            <option value="Programming Language" {{ old('category', $skill->category) == 'Programming Language' ? 'selected' : '' }}>Programming Language</option>
    ...
    <div>
        ...
        <input type="number" name="order" value="{{ old('order', $skill->order) }}" min="0" class="w-full px-4 py-3 bg-dark border border-primary/30 rounded-lg text-light placeholder-gray-400 focus:outline-none focus:border-primary transition-colors duration-300">
    </div>
